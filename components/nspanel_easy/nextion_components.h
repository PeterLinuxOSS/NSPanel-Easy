...
struct NSPanelEasyNextionComponent {
 ...
 * @return A NSPanelEasyNextionComponent struct with the extracted or default page name, component ID, and current page
 * status.
 */
 ...
 */
template<typename Container> inline uint16_t rgbTo565(const Container &rgb) {
  if (rgb.size() != 3) {
